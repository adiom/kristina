...
const mockedSearchUserMemory = jest.mocked(searchUserMemory);
...
describe('MCP Streamable HTTP route', () => {
  ...
  it('lists the public Kristina tools', async () => {
    const response = await POST(
      mcpRequest(
    ...
    expect(body.result.tools.map((tool: { name: string }) => tool.name)).toEqual(
      ['agent_info', 'agent_message', 'agent_search'],
    );
  });
  ...
  it('validates and forwards agent_message arguments', async () => {
    ...
    expect(response.status).toBe(200);
    expect(mockedProcessAgent).toHaveBeenCalledWith('Привет', context);
  ...
  it('searches only namespaces enabled by the context', async () => {
    ...
    mockedSearchUserMemory.mockResolvedValue([{ id: 'user-1' }] as never);
    ...
    expect(mockedSearchUserMemory).not.toHaveBeenCalled();
